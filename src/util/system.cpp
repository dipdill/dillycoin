...
const char * const BITCOIN_CONF_FILENAME = "dillycoin.conf";
...
#else
    const char* pszModule = "dillycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Dillycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Dillycoin";
#else
    // Unix
    return pathRet / ".dillycoin";
#endif
#endif
}
