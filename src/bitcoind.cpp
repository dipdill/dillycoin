...
static bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/dillycoin.conf are parsed in qt/dillycoin.cpp's main()
    SetupServerArgs();
    std::string error;
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  dillycoind [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                tfm::format(std::cerr, "Error: Command line contains unexpected token '%s', see dillycoind -h for a list of options.\n", argv[i]);
                return false;
            }
        }
...
#endif
            tfm::format(std::cout, "Dillycoin server starting\n");
