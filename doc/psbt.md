# PSBT Howto for Dillycoin Core

Since Dillycoin Core 0.17, an RPC interface exists for Partially Signed Dillycoin
...
PSBT is an interchange format for Dillycoin transactions that are not fully signed
...
Overall, the construction of a fully signed Dillycoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Dillycoin transaction (in network format)
...
#### Multisig with multiple Dillycoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Dillycoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `dillycoin-cli` is used.
