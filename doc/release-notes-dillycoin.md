Dillycoin Core version 0.18.1 is now available from:

  <https://download.dillycoin.com/dillycoin-0.18.1/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/dipdill/dillycoin/issues>

How to Upgrade
==============

As this is the first version of Dillycoin Core forked from the Litecoin Core
there are no upgrade procedures at this time. 

Compatibility
==============

Dillycoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Dillycoin Core on unsupported systems.

Dillycoin Core should also work on most other Unix-like systems but is not
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/dipdill/dillycoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:12332:12332` (this is an extra `:12332` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@dillycoin.com) the Dillycoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 DIL),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in DIL per kilobyte, not diddy per byte.

Documentation
-------------

- A new short [document](https://github.com/dipdill/dillycoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/dipdill/dillycoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/dipdill/dillycoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/dipdill/dillycoin/blob/master/doc/dillycoin-conf.md)
  about the `dillycoin.conf` file describes how to use it to configure
  Dillycoin Core.

- A new document introduces Dillycoin Core's BIP174 [Partially-Signed
  Dillycoin Transactions
  (PSBT)](https://github.com/dipdill/dillycoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/dipdill/dillycoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Dillycoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Dillycoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/dipdill/dillycoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/dipdill/dillycoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Dillycoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Dillycoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/dipdill/dillycoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/dipdill/dillycoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart dillycoind with the `-deprecatedrpc=generate`
...
- A new `dillycoin-wallet` tool is now distributed alongside Dillycoin
...
This section describes planned changes to Dillycoin Core that may affect
other Dillycoin software and services.

- Since version 0.16.0, Dillycoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Dillycoin Core 0.20 (expected about
  a year after 0.18), Dillycoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Dillycoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Dillycoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Dillycoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Dillycoin Core
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Dillycoin Core's own implementation, although entropy
  gathered by Dillycoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Dillycoin Core a
...
- On macOS, Dillycoin Core now opts out of application CPU throttling
